...
const RegisterTeams = () => {
  ...
  const handleCsvUpload = async (e) => {
    ...
    try {
      const result = await batchUploadTeams(csvFile, id)
      
      // Show a summary of the results, including any errors
      if (result.totalCreated > 0) {
        if (result.totalErrors > 0) {
          // Some teams were created, but some had errors
          setError(`Imported ${result.totalCreated} teams successfully, but ${result.totalErrors} teams had errors. Problems: ${result.errors.map(e => `Row ${e.row} (${e.teamName}): ${e.error}`).join('; ')}`)
        } else {
          // All teams were created successfully
          setError(null)
          setSuccessMessage(`Successfully imported ${result.totalCreated} teams!`)
        }
      } else if (result.totalErrors > 0) {
        // No teams were created, all had errors
        setError(`Failed to import any teams. Problems: ${result.errors.map(e => `Row ${e.row} (${e.teamName}): ${e.error}`).join('; ')}`)
      }
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
          ...
          <div className="mt-10 pt-8 border-t border-gray-200">
            ...
            <form onSubmit={handleCsvUpload} className="space-y-4">
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Expected format:</span> team, institute, member1, member1-email, member2, member2-email
                  </p>
                  <p className="text-sm text-gray-500">
                    Column headers are flexible - we'll match "team", "team name", "Team Name", etc. Only team name is required for bulk import.
                  </p>
                  <div className="bg-blue-50 p-3 rounded-md mt-2">
                    <h4 className="text-sm font-medium text-blue-800">CSV Example:</h4>
                    <pre className="text-xs text-blue-700 mt-1 whitespace-pre-wrap">
                      team,institute,member1,member1-email,member2,member2-email
                      <br/>Team Alpha,Stanford University,John Doe,john@example.com,Jane Smith,jane@example.com
                      <br/>Team Beta,Harvard University,Alex Johnson,alex@example.com,Chris Wilson,chris@example.com
                    </pre>
                  </div>
                </div>
              </div>
              ...
              <button
                type="submit"
                className={`w-full py-3 px-4 ${csvLoading ? 'bg-gray-500' : 'bg-green-600 hover:bg-green-700'} text-white font-medium rounded-lg transition duration-200 flex items-center justify-center`}
                disabled={csvLoading}
              >
                {csvLoading ? (
                  <>
              ...
              {teams.map((team) => (
                <div key={team.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition duration-200">
                  ...
                  <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-700"><span className="font-medium">Member 1:</span> {team.member_1_name || 'None'}</p>
                      {team.member_1_email && <p className="text-sm text-gray-500">{team.member_1_email}</p>}
                    </div>
                    <div>
                      <p className="text-sm text-gray-700"><span className="font-medium">Member 2:</span> {team.member_2_name || 'None'}</p>
                      {team.member_2_email && <p className="text-sm text-gray-500">{team.member_2_email}</p>}
                    </div>
                  </div>
                </div>
              ))}
